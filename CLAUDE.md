...
- `screens/` - one `scr_*` module per UI screen (`scr_startup`, `scr_menu`, `scr_play`, `scr_ranking`, `scr_setting`) plus shared `screens_bitmap.*` bitmap data. `scr_play` is the game screen - it drives the gameplay loop and fans signals out to the game-object tasks (see the sequence diagram in `resource/guide/03-design-sequence-object.md`).
