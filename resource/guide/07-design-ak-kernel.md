...
Neither is mandatory. In this project `tsm` drives the four game objects in `app/tiny_rex_game/` (one table per state, each row mapping a signal to a next state and an action): `tiny_rex_object` (`RUNNING`/`JUMPING`/`FALLING`/`DUCKING`), `obstacles_object` (`RUNNING`/`COLLIDED`), and `horizon_object` and `score` (a single state each, so their table is just a signal-to-action map). It also drives the idle screen `scr_idle` (`RUNNING`/`JUMPING` for the demo T-Rex). `fsm` has no user. The other screens, including `scr_menu`, use a plain `switch` on the signal behind `screen_manager`'s dispatch.
